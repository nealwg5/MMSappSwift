...
class NEJMArticles {
    ...
    //Gets all article information for the given date range, returns an array of the article objects
    func getArticles(dateRange: Int) -> [Article] {
        ...
        if ((theCache.getCachedArticleTopicSetting().articleTopic != "All topics") && (theCache.getCachedArticleTopicSetting().isData == true)){
            ...
            url = "https://ilerxy5ujh.execute-api.us-east-1.amazonaws.com/nejm_stage/search?query=topic:%22\(theCache.getCachedArticleTopicSetting().articleTopic)%22__has_oa_conclusions:%22true%22&startDate=\(contentDate.fullDate)&endDate=\(endDate)&format=xml"
            
        } else {        //Don't include a specific article topic in the search
            
            url = "https://ilerxy5ujh.execute-api.us-east-1.amazonaws.com/nejm_stage/search?query=has_oa_conclusions:%22true%22&startDate=\(contentDate.fullDate)&endDate=\(endDate)&format=xml"
